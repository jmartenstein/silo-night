...
RSpec.describe 'Local-First Search Strategy', type: :integration do
  it 'includes shows from the database in the search results', vcr: { record: :once } do
    ...
    ShowMetadata.create(show_id: s.id, provider_name: 'internal', external_id: s.name, payload: { poster_path: '/poster.jpg' })
  ...
  it 'returns both local and API results when they are distinct shows' do
    # Local show
    s = Show.create(name: 'The Show')
    ShowMetadata.create(show_id: s.id, provider_name: 'internal', external_id: s.name, payload: { poster_path: '/local.jpg' })
